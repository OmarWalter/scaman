...
const HERO_FX_SELECTORS =
  ".hero-title, .hero-subtitle, .hero-char, .hero-buttons, .hero-ticker, .scroll-indicator, .gate-text, .gate-sub";
...
gateOverlay &&
  gateOverlay.addEventListener("click", () => {
    ...
    tl.to(".gate-text", { scale: 1.3, duration: 0.18, ease: "power2.in" })
      ...
      .from(
        ".hero-char",
        {
          ...
          onComplete: () => {
            ...
            setTimeout(
              () => window.startMoneyRain && window.startMoneyRain(),
              900
            );
          },
        },
      ...
      .from(
        ".scroll-indicator",
        { opacity: 0, y: 10, duration: 0.4, clearProps: "all" },
        "-=0.2"
      );
  });
...
document
  .querySelectorAll("a, button, .about-card, .scenario, .gallery-item")
  .forEach((el) => {
...
let lastHoverEmit = 0;
document
  .querySelectorAll(".gallery-item, .about-card, .scenario, .hero-char")
  .forEach((el) => {
...
try {
  if (typeof ScrollTrigger !== "undefined") {
    ...
    document
      .querySelectorAll(".reveal-text, .reveal-up, .reveal-left, .reveal-right")
